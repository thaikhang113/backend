...
const Booking = {
    ...
    addRoom: async (bookingId, roomId, price) => {
        await db.query(
    ...
    getBookedRooms: async (bookingId) => {
        const res = await db.query(`
    ...
    updateStatus: async (id, status) => {
        const res = await db.query('UPDATE Bookings SET status = $1 WHERE booking_id = $2 RETURNING *', [status, id]);
        return res.rows[0];
    }
};
